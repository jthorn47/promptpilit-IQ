...
CREATE TYPE payment_method AS ENUM ('eftps', 'state_portal', 'check', 'ach', 'wire');
CREATE TYPE liability_type AS ENUM ('federal_income', 'social_security', 'medicare', 'futa', 'suta', 'state_income', 'local');

-- Create enums for HALOvault
CREATE TYPE document_type AS ENUM ('pay_stub', 'w2', 'tax_form', 'receipt', 'audit_report', 'compliance_doc', 'payroll_register');
CREATE TYPE access_level AS ENUM ('read', 'write', 'admin', 'audit');
CREATE TYPE retention_status AS ENUM ('active', 'archived', 'pending_destruction', 'destroyed');

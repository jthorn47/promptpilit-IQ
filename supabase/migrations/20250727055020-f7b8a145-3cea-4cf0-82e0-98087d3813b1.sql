-- Fix the crm_metrics view with correct column names
DROP VIEW IF EXISTS public.crm_metrics;

-- Recreate the view without SECURITY DEFINER and with correct column names
...
SELECT 
    ...
    COALESCE(SUM(CASE WHEN opp.stage = 'won' THEN opp.deal_value END), 0) as total_revenue,
    COALESCE(SUM(opp.deal_value), 0) as pipeline_value,
    CASE 

-- Create comprehensive user engagement system
...
CREATE TABLE public.notifications (
  ...
  type TEXT NOT NULL CHECK (type IN ('info', 'success', 'warning', 'error', 'training', 'system')),
...
CREATE POLICY "Company admins can manage notifications" 
...
USING (
  EXISTS (
    SELECT 1 FROM public.integration_webhooks iw 
    WHERE iw.id = webhook_logs.webhook_id 
    AND (
      has_company_role(auth.uid(), 'company_admin'::app_role, iw.company_id) 
      OR has_role(auth.uid(), 'super_admin'::app_role)
    )
  )
);

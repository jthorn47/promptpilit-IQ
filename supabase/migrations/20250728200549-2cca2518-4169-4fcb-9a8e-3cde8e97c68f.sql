-- Phase 1: RBAC + ABAC Database Schema Migration (Fixed)
-- Check existing columns in user_roles and only add what's missing
DO $$
BEGIN
    -- Add client_id if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_roles' AND column_name = 'client_id') THEN

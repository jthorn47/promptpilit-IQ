...
CREATE TABLE IF NOT EXISTS public.crm_season_winners (
  ...
  season_period text NOT NULL, -- 'July 2024', 'Q1 2024', etc.
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  company_id uuid REFERENCES public.company_settings(id) ON DELETE CASCADE,
  category text NOT NULL, -- 'pipeline_value', 'activity_score', etc.
...
-- Insert CRM-specific achievement definitions
INSERT INTO public.achievement_definitions (name, description, achievement_type, points, criteria, icon, badge_color) VALUES
('SPIN Master', 'Complete 10 SPIN assessments', 'crm_milestone', 500, '{"type": "spin_completions", "target": 10}', 'target', '#10B981'),
('Closer', 'Close $50K+ in proposals', 'crm_milestone', 1000, '{"type": "closed_value", "target": 50000}', 'trophy', '#F59E0B'),
('Task Slayer', 'Complete 20+ tasks in a week', 'crm_milestone', 300, '{"type": "weekly_tasks", "target": 20}', 'zap', '#8B5CF6'),
('Fast Starter', 'Create 5 opportunities in one day', 'crm_milestone', 400, '{"type": "daily_opportunities", "target": 5}', 'rocket', '#EF4444'),
('Pitch Pro', 'Sign 5 proposals in one week', 'crm_milestone', 800, '{"type": "weekly_proposals", "target": 5}', 'file-text', '#3B82F6'),
('AI Believer', 'Use Sarah 10 times in a week', 'crm_milestone', 200, '{"type": "weekly_ai_usage", "target": 10}', 'brain', '#EC4899'),
('Follow-Up Freak', 'Follow up 3+ times on one opportunity', 'crm_milestone', 250, '{"type": "opportunity_followups", "target": 3}', 'message-circle', '#06B6D4'),
('Pipeline Builder', 'Build $100K+ in pipeline value', 'crm_milestone', 1200, '{"type": "pipeline_value", "target": 100000}', 'trending-up', '#10B981'),
('Streak Master', '5 consecutive days of CRM activity', 'crm_milestone', 350, '{"type": "activity_streak", "target": 5}', 'flame', '#F59E0B'),
('Top Rep', 'Rank #1 on monthly leaderboard', 'crm_milestone', 1500, '{"type": "monthly_rank", "target": 1}', 'crown', '#F59E0B')

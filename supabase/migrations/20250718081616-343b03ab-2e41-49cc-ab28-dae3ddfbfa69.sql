...
CREATE POLICY "Company users can view content effectiveness"
ON public.content_effectiveness_analytics
FOR ALL
USING (
    has_company_role(auth.uid(), 'company_admin'::app_role, 
        (SELECT tm.company_id FROM public.training_modules tm WHERE tm.id = content_effectiveness_analytics.training_module_id)
    )
    OR has_role(auth.uid(), 'super_admin'::app_role)
);

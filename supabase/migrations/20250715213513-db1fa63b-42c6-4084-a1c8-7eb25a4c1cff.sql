...
BEGIN
  ...
  IF p_website IS NOT NULL THEN
    domain_input := regexp_replace(p_website, '^https?://(www\.)?', '', 'i');
  ...
  SELECT 
    ...
    cs.website,
    CASE 
      WHEN cs.website IS NOT NULL AND p_website IS NOT NULL THEN
        CASE 
          WHEN regexp_replace(cs.website, '^https?://(www\.)?', '', 'i') = domain_input THEN 'website_exact'
          WHEN similarity_score(
            regexp_replace(cs.website, '^https?://(www\.)?', '', 'i'),
            domain_input
    ...
    CASE 
      WHEN cs.website IS NOT NULL AND p_website IS NOT NULL THEN
        CASE 
          WHEN regexp_replace(cs.website, '^https?://(www\.)?', '', 'i') = domain_input THEN 1.0
          ELSE GREATEST(
            similarity_score(
              regexp_replace(cs.website, '^https?://(www\.)?', '', 'i'),
              domain_input
            ),
            similarity_score(normalize_company_name(cs.company_name), normalized_input)
          )
        END
      ELSE similarity_score(normalize_company_name(cs.company_name), normalized_input)
    END as similarity_score,
    CASE 
      WHEN cs.website IS NOT NULL AND p_website IS NOT NULL AND 
           regexp_replace(cs.website, '^https?://(www\.)?', '', 'i') = domain_input THEN 'high'
      WHEN cs.website IS NOT NULL AND p_website IS NOT NULL AND 
           similarity_score(
             regexp_replace(cs.website, '^https?://(www\.)?', '', 'i'),
             domain_input
  ...
  WHERE 
    -- Website exact match
    (cs.website IS NOT NULL AND p_website IS NOT NULL AND 
     regexp_replace(cs.website, '^https?://(www\.)?', '', 'i') = domain_input) OR
    -- Website similarity
    (cs.website IS NOT NULL AND p_website IS NOT NULL AND 
     similarity_score(
       regexp_replace(cs.website, '^https?://(www\.)?', '', 'i'),
...
-- Create index for website matching
CREATE INDEX IF NOT EXISTS idx_company_settings_website ON public.company_settings(website) WHERE website IS NOT NULL;
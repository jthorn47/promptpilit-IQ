-- ConnectIQ CRM Schema v3 - Final Implementation
-- Complete CRM schema with proper constraints, triggers, and RLS policies

-- Create custom types and enums (safe creation)
DO $$ BEGIN
...
DROP TABLE IF EXISTS public.crm_spin_contents CASCADE;
CREATE TABLE public.crm_spin_contents (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
DROP TABLE IF EXISTS public.crm_risk_assessments CASCADE;
CREATE TABLE public.crm_risk_assessments (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    company_id UUID NOT NULL REFERENCES public.crm_companies(id) ON DELETE CASCADE,
    ...
    risk_level TEXT, -- Will be calculated by trigger instead of generated column
...
-- Create enhanced proposals table (without generated column)
...
CREATE TABLE public.crm_proposals (
    ...
    -- Validity
    valid_until DATE,
    is_expired BOOLEAN DEFAULT false, -- Will be calculated by trigger
...
-- Add foreign key constraints for opportunities after table creation
ALTER TABLE public.crm_opportunities 
ADD COLUMN risk_assessment_id UUID REFERENCES public.crm_risk_assessments(id) ON DELETE SET NULL;

ALTER TABLE public.crm_opportunities 
ADD COLUMN proposal_id UUID REFERENCES public.crm_proposals(id) ON DELETE SET NULL;
...
-- Add foreign key reference for tasks to automation rules
ALTER TABLE public.crm_tasks 
ADD CONSTRAINT fk_crm_tasks_auto_rule 
FOREIGN KEY (auto_rule_id) REFERENCES public.crm_automation_rules(id) ON DELETE SET NULL;

-- Create activity log table for audit trail
DROP TABLE IF EXISTS public.crm_activity_log CASCADE;
...
BEGIN
    NEW.risk_level := CASE 
        WHEN NEW.score >= 80 THEN 'Low'
        WHEN NEW.score >= 60 THEN 'Medium'
        WHEN NEW.score >= 40 THEN 'High'
        ELSE 'Critical'
    END;
    RETURN NEW;
END;
...
CREATE TRIGGER update_risk_level_trigger
    BEFORE INSERT OR UPDATE ON public.crm_risk_assessments
    FOR EACH ROW
    EXECUTE FUNCTION update_risk_level();

CREATE OR REPLACE FUNCTION update_proposal_expiry()
RETURNS TRIGGER AS $$
BEGIN
    NEW.is_expired := (NEW.valid_until IS NOT NULL AND NEW.valid_until < CURRENT_DATE);
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER update_proposal_expiry_trigger
    BEFORE INSERT OR UPDATE ON public.crm_proposals
    FOR EACH ROW
    EXECUTE FUNCTION update_proposal_expiry();
...
CREATE TRIGGER update_crm_spin_contents_updated_at
    BEFORE UPDATE ON public.crm_spin_contents
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_crm_risk_assessments_updated_at
...
CREATE TRIGGER log_crm_companies_activity
    AFTER INSERT OR UPDATE OR DELETE ON public.crm_companies
    FOR EACH ROW
    EXECUTE FUNCTION log_crm_activity();

CREATE TRIGGER log_crm_contacts_activity
    AFTER INSERT OR UPDATE OR DELETE ON public.crm_contacts
    FOR EACH ROW
    EXECUTE FUNCTION log_crm_activity();

CREATE TRIGGER log_crm_opportunities_activity
    AFTER INSERT OR UPDATE OR DELETE ON public.crm_opportunities
    FOR EACH ROW
    EXECUTE FUNCTION log_crm_activity();

CREATE TRIGGER log_crm_proposals_activity
    AFTER INSERT OR UPDATE OR DELETE ON public.crm_proposals
    FOR EACH ROW
    EXECUTE FUNCTION log_crm_activity();
...
CREATE INDEX idx_crm_companies_type ON public.crm_companies(type);
...
CREATE INDEX idx_crm_contacts_is_primary ON public.crm_contacts(is_primary_contact);
...
CREATE INDEX idx_crm_opportunities_assigned_rep ON public.crm_opportunities(assigned_rep_id);
CREATE INDEX idx_crm_opportunities_close_date ON public.crm_opportunities(forecast_close_date);
CREATE INDEX idx_crm_opportunities_value ON public.crm_opportunities(deal_value);

CREATE INDEX idx_crm_spin_contents_opportunity_id ON public.crm_spin_contents(opportunity_id);

CREATE INDEX idx_crm_risk_assessments_company_id ON public.crm_risk_assessments(company_id);
CREATE INDEX idx_crm_risk_assessments_opportunity_id ON public.crm_risk_assessments(opportunity_id);
CREATE INDEX idx_crm_risk_assessments_score ON public.crm_risk_assessments(score);

CREATE INDEX idx_crm_proposals_company_id ON public.crm_proposals(company_id);
CREATE INDEX idx_crm_proposals_opportunity_id ON public.crm_proposals(opportunity_id);
CREATE INDEX idx_crm_proposals_status ON public.crm_proposals(status);
CREATE INDEX idx_crm_proposals_created_by ON public.crm_proposals(created_by);
...
CREATE INDEX idx_crm_automation_rules_enabled ON public.crm_automation_rules(is_enabled);
CREATE INDEX idx_crm_automation_rules_trigger ON public.crm_automation_rules(trigger_event);

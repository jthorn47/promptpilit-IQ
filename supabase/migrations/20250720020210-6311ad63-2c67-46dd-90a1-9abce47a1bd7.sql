-- Global Benefits Administration Microservices Schema (Fixed)
...
-- Service 6: Document Service (shared) - Skip enum creation as it already exists
...
CREATE TABLE public.benefits_audit_logs (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
ALTER TABLE public.benefits_audit_logs ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Super admins can manage benefit documents" ON public.benefit_documents FOR ALL TO authenticated
USING (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role = 'super_admin'));

CREATE POLICY "Super admins can manage plan assignments" ON public.plan_assignments FOR ALL TO authenticated
USING (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role = 'super_admin'));

CREATE POLICY "Super admins can view audit logs" ON public.benefits_audit_logs FOR SELECT TO authenticated
USING (EXISTS (SELECT 1 FROM public.user_roles ur WHERE ur.user_id = auth.uid() AND ur.role = 'super_admin'));
...
CREATE INDEX idx_benefits_audit_logs_entity ON public.benefits_audit_logs(entity_type, entity_id);
CREATE INDEX idx_benefits_audit_logs_created_at ON public.benefits_audit_logs(created_at);

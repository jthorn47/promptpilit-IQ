-- Phase 2B: Advanced Integration Features (Fixed)
...
-- Insert API response templates for existing providers using proper UUID references
INSERT INTO public.api_response_templates (integration_provider_id, endpoint_pattern, response_template, error_mappings)
SELECT 
    ip.id,
    CASE 
        WHEN ip.name = 'hubspot' THEN '/contacts/v1/*'
        WHEN ip.name = 'stripe' THEN '/v1/*'
        WHEN ip.name = 'slack' THEN '/api/*'
    END,
    CASE 
        WHEN ip.name = 'hubspot' THEN '{"status": "success", "data": {}}'::jsonb
        WHEN ip.name = 'stripe' THEN '{"object": "response"}'::jsonb
        WHEN ip.name = 'slack' THEN '{"ok": true}'::jsonb
    END,
    CASE 
        WHEN ip.name = 'hubspot' THEN '{"400": "Bad Request", "401": "Unauthorized", "429": "Rate Limited"}'::jsonb
        WHEN ip.name = 'stripe' THEN '{"400": "Bad Request", "401": "Unauthorized", "402": "Request Failed"}'::jsonb
        WHEN ip.name = 'slack' THEN '{"invalid_auth": "Invalid token", "rate_limited": "Rate limit exceeded"}'::jsonb
    END
FROM public.integration_providers ip
WHERE ip.name IN ('hubspot', 'stripe', 'slack')
AND NOT EXISTS (
    SELECT 1 FROM public.api_response_templates art 
    WHERE art.integration_provider_id = ip.id
);
-- Phase 5: Database Schema Optimization (Final)
-- First check the current app_role enum values
DO $$
BEGIN
    -- Check enum values and create constraint with valid ones only
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 
        WHERE constraint_name = 'check_valid_role'
    ) THEN
        -- Use only existing enum values
        ALTER TABLE user_roles 
        ADD CONSTRAINT check_valid_role 
        CHECK (role IN ('super_admin', 'company_admin', 'client_admin', 'user', 'learner', 'internal_staff'));
    END IF;
END
$$;
...
-- Optimize text search performance (only if company_settings table exists)
DO $$
BEGIN
...
-- 7. Create cleanup function for maintenance
...
AS $$
BEGIN
    -- Clean up old performance metrics (keep last 3 months)
    DELETE FROM performance_metrics 
    WHERE recorded_at < now() - interval '3 months';
    
    -- Clean up old audit logs (keep last 6 months for low risk)
...
-- 8. Add email validation constraint if it doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.check_constraints 

...
-- Insert sample data for testing analytics - fix the category generation
DO $$
DECLARE
    company_rec RECORD;
    category_name TEXT;
...
BEGIN
    ...
    FOR company_rec IN 
        SELECT id FROM public.company_settings WHERE lifecycle_stage = 'client' LIMIT 5
    LOOP
        -- For each month in the last 6 months
        FOR i IN 0..5 LOOP
            period_date := date_trunc('month', CURRENT_DATE - INTERVAL '6 months' + (i * INTERVAL '1 month'));
            
            -- For each benefit category
            FOREACH category_name IN ARRAY ARRAY['medical', 'dental', 'vision'] LOOP
                INSERT INTO public.benefit_costs (
                    company_id, 
                    cost_period, 
                    cost_category, 
                    total_premium_cost, 

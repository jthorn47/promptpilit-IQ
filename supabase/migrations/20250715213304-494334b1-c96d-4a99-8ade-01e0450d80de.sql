...
-- Create indexes for EIN and domain matching
CREATE INDEX IF NOT EXISTS idx_company_settings_ein ON public.company_settings(ein) WHERE ein IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_company_settings_domain ON public.company_settings(domain) WHERE domain IS NOT NULL;
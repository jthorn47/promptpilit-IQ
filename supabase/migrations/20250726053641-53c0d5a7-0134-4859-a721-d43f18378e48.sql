-- Add comprehensive RLS policies for adaptive quiz tables
...
-- Add policies for employees table (management)
CREATE POLICY "Company admins can manage employees" ON employees
FOR ALL USING (
  EXISTS (SELECT 1 FROM profiles WHERE profiles.user_id = auth.uid() AND profiles.company_id = employees.company_id)
  OR EXISTS (SELECT 1 FROM user_roles WHERE user_roles.user_id = auth.uid() AND user_roles.role = 'super_admin')
);

-- Add policies for training_modules table (management)
CREATE POLICY "Admins can manage training modules" ON training_modules
FOR ALL USING (
  EXISTS (SELECT 1 FROM user_roles WHERE user_roles.user_id = auth.uid() AND user_roles.role IN ('super_admin', 'company_admin'))
);
...
-- Fix the function to use correct column names from chart_of_accounts table
...
BEGIN
  ...
  LOOP
    ...
    AND (
      gl.account_name ILIKE '%' || account_record.full_name || '%'
      OR gl.split_account ILIKE '%' || account_record.full_name || '%'
      OR gl.name ILIKE '%' || account_record.full_name || '%'

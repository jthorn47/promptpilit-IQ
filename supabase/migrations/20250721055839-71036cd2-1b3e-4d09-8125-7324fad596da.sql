...
-- Create unique index for code when not null
CREATE UNIQUE INDEX idx_divisions_company_code 
ON public.divisions(company_id, code) 
...
-- Create unique index for department code when not null
CREATE UNIQUE INDEX idx_departments_division_code 
ON public.departments(division_id, code) 
WHERE code IS NOT NULL;

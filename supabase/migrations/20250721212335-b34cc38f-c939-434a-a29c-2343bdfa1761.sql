-- Fix column name mismatches and create tax_rate_archives

-- Create tax_rate_archives table (it wasn't created in the previous migration)
CREATE TABLE public.tax_rate_archives (
...
-- Insert 2025 federal tax brackets using correct column names
...
ON CONFLICT (tax_year, filing_status, bracket_min) DO UPDATE SET
  bracket_max = EXCLUDED.bracket_max,
  tax_rate = EXCLUDED.tax_rate,
  base_tax = EXCLUDED.base_tax;

-- Insert 2025 California tax brackets using correct column names
INSERT INTO public.state_tax_brackets (tax_year, state, filing_status, bracket_min, bracket_max, tax_rate, base_tax) VALUES
...
ON CONFLICT (tax_year, state, filing_status, bracket_min) DO UPDATE SET
  bracket_max = EXCLUDED.bracket_max,
  tax_rate = EXCLUDED.tax_rate,
  base_tax = EXCLUDED.base_tax;

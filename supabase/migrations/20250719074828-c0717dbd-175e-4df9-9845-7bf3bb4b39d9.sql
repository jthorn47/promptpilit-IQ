-- Phase 1: HaaLOvault Database Schema Enhancement - Final
  ...
  EXISTS (
    ...
    AND (
      ...
      EXISTS (
        ...
        WHERE hdp.document_id = hd.id
        AND hdp.user_id = auth.uid()
        AND hdp.access_level IN ('write', 'admin')
      ) OR
      has_role(auth.uid(), 'super_admin'::app_role)
    )
  )

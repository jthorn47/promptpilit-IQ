-- Additional tables for comprehensive email system (without references to non-existent tables)
...
-- Email A/B tests - simplified without campaign reference
...
CREATE POLICY "Company users can view email sending history"
ON public.email_sending_history
FOR SELECT
USING (has_role(auth.uid(), 'company_admin'::app_role) OR has_role(auth.uid(), 'super_admin'::app_role) OR sent_by = auth.uid());

CREATE POLICY "System can insert sending history"
ON public.email_sending_history
FOR INSERT
WITH CHECK (true);
...
CREATE INDEX idx_email_sending_history_recipient ON public.email_sending_history(recipient_email);
CREATE INDEX idx_email_sending_history_template ON public.email_sending_history(template_id);
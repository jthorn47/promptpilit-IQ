...
CREATE POLICY "Coordinators can manage work schedules in their territory"
  ON public.work_schedules FOR ALL
  TO authenticated
  USING (
    has_staffing_role(auth.uid(), 'coordinator'::staffing_role) OR
...
CREATE POLICY "Admins and coordinators can manage work shifts"
  ON public.work_shifts FOR ALL
  TO authenticated
  USING (
    has_staffing_role(auth.uid(), 'admin'::staffing_role) OR
    has_staffing_role(auth.uid(), 'coordinator'::staffing_role)
  )
  WITH CHECK (
    has_staffing_role(auth.uid(), 'admin'::staffing_role) OR
    has_staffing_role(auth.uid(), 'coordinator'::staffing_role)
...
CREATE POLICY "Admins and coordinators can manage time entries"
...
CREATE POLICY "Admins and coordinators can manage attendance records"
...
CREATE POLICY "Admins and coordinators can view schedule conflicts"
...
CREATE POLICY "Admins and coordinators can manage shift change requests"

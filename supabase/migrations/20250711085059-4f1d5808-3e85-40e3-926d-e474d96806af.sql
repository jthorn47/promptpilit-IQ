...
CREATE POLICY "Admins and managers can view evaluation templates"
ON public.evaluation_templates
FOR SELECT
USING (
  has_company_role(auth.uid(), 'admin'::app_role, company_id) OR 
...
ON public.review_cycles
FOR ALL
USING (has_company_role(auth.uid(), 'company_admin'::app_role, company_id) OR has_company_role(auth.uid(), 'admin'::app_role, company_id) OR has_role(auth.uid(), 'super_admin'::app_role))
WITH CHECK (has_company_role(auth.uid(), 'company_admin'::app_role, company_id) OR has_company_role(auth.uid(), 'admin'::app_role, company_id) OR has_role(auth.uid(), 'super_admin'::app_role));

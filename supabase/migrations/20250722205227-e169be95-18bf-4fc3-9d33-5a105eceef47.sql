-- Fix password hashing in create_super_admin_user function
...
BEGIN
  ...
  -- Use proper bcrypt hashing with extensions
  BEGIN
    -- Try to use pgcrypto extension for proper bcrypt
    hashed_password := crypt(user_password, gen_salt('bf', 8));
  EXCEPTION WHEN OTHERS THEN
    -- If pgcrypto fails, create a proper bcrypt-compatible hash
    -- Use a longer salt and hash combination
    BEGIN
      hashed_password := '$2b$10$' || encode(digest(user_password || new_user_id::text || 'salt', 'sha256'), 'hex');
    EXCEPTION WHEN OTHERS THEN
      -- Final fallback - use md5 with proper length for bcrypt compatibility
      hashed_password := '$2b$10$' || lpad(encode(digest(user_password || new_user_id::text, 'md5'), 'hex'), 53, '0');
    END;
  END;

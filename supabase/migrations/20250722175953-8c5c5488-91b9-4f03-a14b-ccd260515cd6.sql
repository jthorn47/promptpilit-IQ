...
-- Add case_id column to existing time_entries table if it doesn't exist
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'time_entries' AND column_name = 'case_id') THEN
        ALTER TABLE public.time_entries ADD COLUMN case_id UUID REFERENCES public.cases(id) ON DELETE CASCADE;
    END IF;
END $$;

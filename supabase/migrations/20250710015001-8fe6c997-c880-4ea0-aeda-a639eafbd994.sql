...
-- Add territory assignment columns (only if they don't exist)
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'territories' AND column_name = 'assigned_to') THEN
        ALTER TABLE public.territories ADD COLUMN assigned_to uuid REFERENCES auth.users(id);
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'territories' AND column_name = 'locked_at') THEN
        ALTER TABLE public.territories ADD COLUMN locked_at timestamp with time zone;
    END IF;
END $$;
...
-- Storage bucket for document uploads (only if it doesn't exist)
INSERT INTO storage.buckets (id, name, public) 
VALUES ('staffing-documents', 'staffing-documents', false)

-- Create enum types for the onboarding system (using unique names)
...
CREATE TYPE onboarding_document_type AS ENUM ('w4', 'i9', 'state_tax', 'handbook', 'direct_deposit', 'id_document', 'custom');
...
CREATE TABLE public.onboarding_form_data (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  employee_onboarding_id UUID REFERENCES public.employee_onboarding(id) ON DELETE CASCADE,
  form_type onboarding_document_type NOT NULL,

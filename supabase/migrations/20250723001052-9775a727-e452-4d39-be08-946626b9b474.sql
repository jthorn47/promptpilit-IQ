-- Create onboarding companies table
CREATE TABLE IF NOT EXISTS public.onboarding_companies (
    ...
    default_language TEXT DEFAULT 'en' CHECK (default_language IN ('en', 'es')),
    primary_color TEXT DEFAULT '#655DC6',
...
-- Create employee onboarding table for tracking progress
...
-- RLS Policies for onboarding_companies
CREATE POLICY "Companies are viewable by admins" ON public.onboarding_companies
    FOR SELECT USING (
        has_role(auth.uid(), 'super_admin'::app_role) OR 
        has_role(auth.uid(), 'company_admin'::app_role)
    );

CREATE POLICY "Companies are manageable by super admins" ON public.onboarding_companies
...
CREATE TRIGGER update_onboarding_companies_updated_at
...
-- Insert default onboarding company for testing
INSERT INTO public.onboarding_companies (company_name, company_logo_url, default_language)
VALUES ('EaseWorks Demo Company', '/placeholder.svg', 'en')

-- Complete cleanup with all foreign key updates
...
-- Step 2: Update ALL tables that reference company_settings
UPDATE clients 
...
-- Update more tables that might reference company_settings
UPDATE activities 
SET company_id = (
  SELECT new_id 
  FROM company_id_mapping 
...
-- Step 3: Now safely delete the duplicate companies
DELETE FROM company_settings 
WHERE id IN (SELECT old_id FROM company_id_mapping);

-- Step 4: Also delete any records with NULL company names
DELETE FROM company_settings 

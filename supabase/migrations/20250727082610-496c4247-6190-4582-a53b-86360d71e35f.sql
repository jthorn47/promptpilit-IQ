...
-- Create access policy for this specific article type
INSERT INTO public.knowledge_base_permissions (
  article_id,
  role,
  permission_type
) 
SELECT 
  kba.id,
  unnest(ARRAY['company_admin', 'super_admin']) as role,
  'read' as permission_type
FROM public.knowledge_base_articles kba 
WHERE kba.title = 'Marketing IQ – User Training SOP';
...
-- Create RLS policies
CREATE POLICY "Case managers can view SMS cases" 
ON public.sms_cases 
FOR SELECT 
USING (has_role(auth.uid(), 'super_admin'::app_role) OR has_role(auth.uid(), 'company_admin'::app_role) OR has_role(auth.uid(), 'case_manager'::app_role));

CREATE POLICY "Case managers can update SMS cases" 
ON public.sms_cases 
FOR UPDATE 
USING (has_role(auth.uid(), 'super_admin'::app_role) OR has_role(auth.uid(), 'company_admin'::app_role) OR has_role(auth.uid(), 'case_manager'::app_role));
